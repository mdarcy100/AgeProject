...
#Image 1	
	...
	#plot(cor(exprs(nkiSub.clust.eset.bad),age_signature))
	...
	points(nki_castgood_cor,pch=20,col='purple')
	legend(5,.3, c(paste("Good Prognosis,","N = ",length(nki_castgood_cor),sep=""), paste("Poor Prognosis,","N = ",length(nki_castbad_cor),sep="")),fill = c("purple", "orange"),title=paste(" "),cex=.9)
...
#image of correlations	
	...
	points(caldas_castgood_cor,pch=20,col='purple')
	legend(5,.25, c(paste("Good Prognosis,","N=",length(caldas_castgood_cor),sep=""), paste("Poor Prognosis,","N=",length(caldas_castbad_cor),sep="")),fill = c("purple", "orange"),title=paste(" "),cex=.9)
...
#BOXPLOTS
	...
	boxplot(caldas_bad_cor)
	#plot(cor(exprs(nkiSub.clust.eset.bad),age_signature))
	
	boxplot(caldas_good_cor)
	#plot(cor(exprs(nkiSub.clust.eset.bad),age_signature))
